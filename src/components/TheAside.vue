<template>
    <transition name="slide-fade">
        <div class="the-aside" v-if="showAside">
            ...
            <ul class="menus">
                <li v-for="(item,index) in listOfSongs" :key="index" :class="{'is-play': id==item.song_id}"  
                @click="toPlay(item.song_id, item.url, item.cover, index, item.song_name, item.lyric)">
                    {{replaceFName(item.song_name)}}
                </li>
            </ul>
        </div>
    </transition>
...
export default {
    ...
    methods: {
        ...
        //播放
        toPlay: function(songId,url,pic,index,name,lyric){
